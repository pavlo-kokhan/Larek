...
namespace Kitchen.Products.OnTable
{
    public class ProductObjectsFactory : IFactory<Product, Vector3, GameObject>
    {
        ...
        public GameObject Create(Product product, Vector3 position)
        {
            ...
            if (instance.TryGetComponent<ProductObject>(out var productObject) == false)
            {
                Debug.LogWarning($"Missing {nameof(ProductObject)} " +
                                 $"or {nameof(ProductObjectPerspectiveChanger)} for {nameof(prefab)}.");
                return null;
            }
